...
async def generate_frames():
    """Asynchronous generator that yields the latest camera frame.
    Copies the frame while holding the lock and then yields outside the lock
    to avoid blocking the camera thread during transmission and sleeping.
    """
    while True:
        with state.lock:
