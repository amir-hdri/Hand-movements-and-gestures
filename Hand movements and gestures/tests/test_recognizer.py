...
class TestGestureRecognizer(unittest.TestCase):
    ...
    def test_critical_threshold(self):
        # Setup with critical action logic
        mock_model = MagicMock()
        recognizer = GestureRecognizer(
            model=mock_model,
            actions=["A", "B", "C"], # A=0, B=1, C=2
            seq_length=3,
            threshold=0.5,
            stable_count=1,
            critical_actions=["B"],
        ...
        # Fill buffer (length 3)
        mock_model.predict.return_value = np.array([[0.0, 0.0, 0.0]]) # dummy
        recognizer.update(fv)
        recognizer.update(fv)

        # Scenario 1: Predicting "A" (non-critical) with 0.6 conf (pass default 0.5)
        mock_model.predict.return_value = np.array([[0.6, 0.2, 0.2]])
        pred = recognizer.update(fv)
        self.assertEqual(pred.raw_action, "A")

        # Scenario 2: Predicting "B" (critical) with 0.8 conf (fail critical 0.9)
        mock_model.predict.return_value = np.array([[0.1, 0.8, 0.1]])
        pred = recognizer.update(fv)
        self.assertIsNone(pred.raw_action)
        self.assertAlmostEqual(pred.confidence, 0.8)

        # Scenario 3: Predicting "B" (critical) with 0.95 conf (pass critical 0.9)
        mock_model.predict.return_value = np.array([[0.05, 0.95, 0.0]])
        pred = recognizer.update(fv)
        self.assertEqual(pred.raw_action, "B")
